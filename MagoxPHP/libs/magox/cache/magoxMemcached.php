...
class magoxMemcached implements icache{
	...
	private function connect(){
		if( 0===itemIsNull(self::$mem) ){
			...
			self::$mem->addServer($this->host['host'],$this->host['port']);
			$this->addHost();
		}
	}
	//增加服务器
	...
	public function set($name,$value,$time=S_CACHE_SAVE_TIME){
		return self::$mem->set($name,$value,$time);
	}
	//增加变量
	public function add($name,$value,$time=S_CACHE_SAVE_TIME){
		return self::$mem->add($name,$value,$time);
	}
	//得到变量
